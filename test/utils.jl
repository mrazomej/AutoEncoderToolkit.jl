...
@testset "vec_mat_vec_tullio tests" begin
    @testset "vector inputs" begin
        v = [1, 2, 3]
        M = [4 5 6; 7 8 9; 10 11 12]
        w = [13, 14, 15]
        expected = LinearAlgebra.dot(v, M, w)
        @test typeof(utils.vec_mat_vec_tullio(v, M, w)) <: Number
        @test utils.vec_mat_vec_tullio(v, M, w) == expected
    end
    ...
    @testset "matrix inputs" begin
        ...
        @test typeof(utils.vec_mat_vec_tullio(v, M, w)) <: AbstractVector
        @test utils.vec_mat_vec_tullio(v, M, w) == expected
    end
    ...
    @testset "dimension mismatch" begin
        ...
        M = [4 5 6; 7 8 9]
        w = [10, 11, 12]
        @test_throws DimensionMismatch utils.vec_mat_vec_tullio(v, M, w)
    end
end
